...
word_occurences %>%
  ...
  coord_flip()
```
Much better. A computer linguist might be able to drop only common phrases from the dataset instead of dropping words alltogether. But we are currently limited to this more blunt approach.
...
word_occurences_per_party_wide %>% head(10)  
...
This plot shows that the AfD is using words like "Bürger", "Merker", "Altparteien", "Regierung", "EU", "Migraten", usw. more often than every other fraction.
